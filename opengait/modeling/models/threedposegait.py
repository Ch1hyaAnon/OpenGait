...
class ThreeDPoseGait(BaseModel):
   ...
   def build_network(self, model_cfg):
       ...
       # Change from 2 to 3 channels for 3D pose (x, y, z coordinates)
       self.map_layer0 = SetBlockWrapper(nn.Sequential(
           conv3x3(3, self.inplanes, 1),  # Changed from 2 to 3 for 3D pose
           nn.BatchNorm2d(self.inplanes),
           nn.ReLU(inplace=True)
       ))
   ...
   def inputs_pretreament(self, inputs):
       ...
       for pose, sil in zip(pose_sils[0], pose_sils[1]):
           ...
           cat_data = np.concatenate([pose, sil], axis=1) # [T, 4, H, W] - 3D pose + silhouette
   ...
   def forward(self, inputs):
       ...
       # Update assertion for 3D pose dimensions - expect wider input due to 3 coordinates
       assert pose.size(-1) in [66, 72, 132, 144]  # Updated for 3D pose (approximately 1.5x of 2D dimensions)
       maps = pose[:, :3, ...]  # Extract first 3 channels (x, y, z coordinates)
       sils = pose[:, -1, ...].unsqueeze(1)  # Extract silhouette (last channel)
